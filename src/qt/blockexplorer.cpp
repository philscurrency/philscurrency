...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PHILS, nValue);
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        Generated = GetBlockValue(0, pBlock->nHeight - 1, 0);
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        {
            itostr(i),
            (HashNext == uint256("0"))? (fAddrIndex? _("no") : _("unknown")) : "<span>" + makeHRef(HashNext.GetHex()) + ":" + itostr(nNext) + "</span>",
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown)
    {
        ...
        {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (philscurrency.conf).");
            QMessageBox::warning(this, "Philscurrency Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
bool BlockExplorer::switchTo(const QString& query)
{
    ...
    uint256 hash = uint256(query.toUtf8().constData());
